...
SELECT
		...
		WHERE TO_CHAR(DR_APPT_TIME, 'YYYY-MM-DD') = '2024-06-20';
   		...
   		ORDER BY "reservationTime", "type", "memberName";
...
SELECT COMMENT_NO, COMMENT_CONTENT, MEMBER_NAME,
			COMMENT_WRITE_DATE, BOARD_NO, PARENT_COMMENT_NO, COMMENT_DEL_FL
		FROM "COMMENT"
		JOIN MEMBER USING(MEMBER_NO)
		WHERE BOARD_NO = 293
...
SELECT LEVEL, C.*
		FROM
			(SELECT COMMENT_NO, COMMENT_CONTENT,
				TO_CHAR(COMMENT_WRITE_DATE, 'YYYY"년" MM"월" DD"일" HH24"시" MI"분" SS"초"') COMMENT_WRITE_DATE,
				BOARD_NO, M.MEMBER_NO, M.MEMBER_NAME, PARENT_COMMENT_NO, COMMENT_DEL_FL
			FROM "COMMENT" C
	...
	SELECT LEVEL, C.*
		FROM
			(SELECT COMMENT_NO, COMMENT_CONTENT,
			TO_CHAR(COMMENT_WRITE_DATE, 'YYYY"년" MM"월" DD"일" HH24"시" MI"분" SS"초"') COMMENT_WRITE_DATE,
			BOARD_NO, C.MEMBER_NO, M.MEMBER_NAME, PARENT_COMMENT_NO, COMMENT_DEL_FL
			FROM "COMMENT" C
			JOIN "MEMBER" M ON C.MEMBER_NO = M.MEMBER_NO
			WHERE BOARD_NO = 293
			<choose>
...
SELECT LEVEL, C.*
	...
	ROLLBACK;
	INSERT INTO "COMMENT"
		VALUES(SEQ_COMMENT_NO.NEXTVAL, 'ㄴㅁㅎㄹ',
			DEFAULT, DEFAULT, 293, 4, NULL, 0, 0);
...
AND MEMBER_NO = 16;
--SET MEMBER_ADDRESS = '01234^^^서울 중구 남대문로^^^비밀'
UPDATE MEMBER
SET CAREGIVERS_NO = 1
, ROOM_NO = 1
WHERE MEMBER_NO = 16;
