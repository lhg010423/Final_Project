...
//     // jSuites 캘린더 인스턴스 생성
//     let instance = jSuites.calendar(document.getElementById('calendar'), {
//         format: 'DD/MM/YYYY HH:MM', // 날짜 형식 지정
//         onupdate: function() {  // 캘린더가 업데이트될 때 호출되는 함수
//             fetchReservedDates(); // 예약된 날짜를 가져오는 함수 호출
//         }
//     });
...
//                 highlightReservedDates(reservedDates); // 예약된 날짜를 강조하는 함수
//             })
...
//         // 예약된 날짜와 일치하는 요소에 클래스 추가 및 클릭 이벤트 추가
//         days.forEach(function(day) {
//             const dayNumber = parseInt(day.innerText, 10); // 날짜 요소의 텍스트를 숫자로 변환
//             if (!isNaN(dayNumber)) {
//                 const selectedDate = new Date(instance.getValue());
//                 const currentMonth = selectedDate.getMonth();
//                 const currentYear = selectedDate.getFullYear();
...
//                     // 클릭 이벤트 추가
//                     day.addEventListener('click', function() {
//                         fetchReservationsForDate(dayNumber); // 특정 날짜의 예약 목록을 가져오는 함수 호출
//                         window.selectedDateValue = `${currentYear}${currentMonth + 1 < 10 ? '0' + (currentMonth + 1) : currentMonth + 1}${dayNumber < 10 ? '0' + dayNumber : dayNumber}`;
//                     });
//                 } else {
//                     // 예약이 없는 날짜에도 클릭 이벤트 추가
//                     day.addEventListener('click', function() {
//                         displayNoReservationsMessage(dayNumber); // 예약이 없는 날짜를 클릭했을 때 메시지를 표시하는 함수 호출
//                         window.selectedDateValue = `${currentYear}${currentMonth + 1 < 10 ? '0' + (currentMonth + 1) : currentMonth + 1}${dayNumber < 10 ? '0' + dayNumber : dayNumber}`;
//                     });
//                 }
//             }
...
//                     // 예약 시간을 기준으로 그룹화
//                     const groupedData = data.reduce((add, curr) => {
//                         const time = curr.reservationTime;
//                         if(!acc[time]) {
//                             add[time] = [];
//                         }
//                         acc[time].push(curr);
...
//                         const reservations = groupedData[time];
//                         const div = document.createElement('div');
//                         div.classList.add('reservation-time-group');
//                         div.innerHTML = `<h3>${time.reservationTime}</h3>`;

//                         reservations.forEach(reservation => {
//                             const resvDiv = document.createElement('div');
//                             resvDiv.classList.add('reservation-item');
//                             resvDiv.innerHTML = `
//                                 <span class="reservation-type">${reservation.type}</span>
//                                 <span class="reservation-member">${reservation.memberName}</span>
//                             `;
//                             div.appendChild(resvDiv);
//                         });
